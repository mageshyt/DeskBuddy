...
TasksScreen::TasksScreen(TFT_eSPI &tft, DashboardState &state) 
...
void TasksScreen::onEnter() {
  ...
  // Align global state.tasks count to tasks list completed items
...
bool TasksScreen::handleInput(InputEvent event) {
  if (event == InputEvent::DPAD_DOWN) {
    ...
    selectedIndex_ = (selectedIndex_ + 1) % TASK_COUNT;
    drawTaskRow(oldIndex, false);
    drawTaskRow(selectedIndex_, true);
    Serial.printf("[TasksScreen] Down -> Selected task %d\n", selectedIndex_);
    return true; // Consumed
  } 
  else if (event == InputEvent::DPAD_UP) {
    uint8_t oldIndex = selectedIndex_;
    selectedIndex_ = (selectedIndex_ + TASK_COUNT - 1) % TASK_COUNT;
    drawTaskRow(oldIndex, false);
    drawTaskRow(selectedIndex_, true);
    Serial.printf("[TasksScreen] Up -> Selected task %d\n", selectedIndex_);
    return true; // Consumed
  } 
    ...
    // Redraw the list
    drawTaskList(true);
    Serial.printf("[TasksScreen] Toggle task %d -> Completed: %s\n", selectedIndex_, tasks_[selectedIndex_].completed ? "YES" : "NO");
    return true; // Consumed
  }
...
void TasksScreen::drawTaskList(bool force) {
  ...
  for (uint8_t i = 0; i < TASK_COUNT; i++) {
    drawTaskRow(i, i == selectedIndex_);
  }
...
void TasksScreen::drawTaskRow(uint8_t index, bool isSelected) {
  ...
  // Draw Checkbox
  const int16_t checkSize = 14;
  const int16_t checkX = rowX + 10;
  ...
  if (tasks_[index].completed) {
    // Fill the checkbox and draw a small cross or inner square
  ...
  tft_.setCursor(checkX + checkSize + 12, rowY + 9);
  tft_.print(tasks_[index].title);
}
